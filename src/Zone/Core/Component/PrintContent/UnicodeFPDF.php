...
define('UnicodeFPDF_VERSION', '1.0.0');
define('FPDF_FONTPATH', dirname(__FILE__) . '/fonts/');

class UnicodeFPDF extends FPDF
{
    ...
    function AddFont($family, $style = '', $file = '')
    {
        ...
        if (defined('FPDF_FONTPATH'))
            $file = FPDF_FONTPATH . strtolower($file);
        $type = null;
        $name = null;
        $desc = null;
        $up = null;
    ...
    function _puttruetypeunicode($font = array())
    {
        //Type0 Font
        $this->_newobj();
        $this->_out('<</Type /Font');
    ...
    function _putinfo()
    {
        $this->_out('/Producer ' . $this->_textstring('UnicodeFPDF ' . UnicodeFPDF_VERSION));
        if (!empty($this->title))
