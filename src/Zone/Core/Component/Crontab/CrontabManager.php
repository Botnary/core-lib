...
namespace Zone\Core\Component\Crontab;
...
class CrontabManager
{
    ...
    protected function _command()
    {
        ...
        $cmd = '';
       /* if ($this->user) {
            $cmd .= sprintf('su -u %s ', $this->user);
        }*/
        $cmd .= escapeshellcmd($this->crontab);
        return $cmd;
    }
    ...
    {
        $out = $this->_exec($this->_command() . ' -l /dev/null 2>&1', $retVal);
        return $out;
    }
    ...
    function clearJobs()
    {
        $this->_exec($this->_command() . ' -r > /dev/null 2>&1', $r);
    }
